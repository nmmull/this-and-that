...
rules we're playing, this is /necessary/. We can demonstrate this
formally by deriving the principle of explosion from the above lemma
without using ~ind-Absurd~.  This works by applying our lemma to a
1-element list [1], using our false assumption to prove that 0 ∈ [1],
